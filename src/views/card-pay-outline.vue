    ...
    <div :class="{ 'px-2 py-4': $vuetify.breakpoint.xs, 'px-6 py-6': $vuetify.breakpoint.smAndUp }">
        ...
        <article class="mb-12">
            ...
            <blockquote class="mb-8 blockquote body-1">
                ...
                <v-icon class="mr-1 purple--text text--accent-4">mdi-circle-small</v-icon><b>브라우저 안에서 결제데이터를 주고받는 ‘즉시결제(PAY)’ 방식</b
                ><br />
                <v-icon class="mr-1 purple--text text--accent-4">mdi-circle-small</v-icon><b>가맹점 서버에서 주고받는 ‘결제요청 재컨펌 (CERT)’ 방식</b><br />
                <v-icon class="mr-1 purple--text text--accent-4">mdi-circle-small</v-icon
                ><b>결제창을 호출하지 않고 REST_API를 통해 기등록된 빌링키로 결제를 요청하는 방식</b><br />
                이 있습니다.<br /><br />
                <v-icon class="mr-1 purple--text text--accent-4">mdi-circle-small</v-icon><b>당장 결제를 내지 않고, 빌링키만 등록하는 빌링키 등록(AUTH) 방식</b
                ><br />
                도 지원합니다.<br /><br />
                빌링키 등록 방식을 통해 가맹점은 결제수단별 빌링키를 사전에 발급받고, 추후 결제를 낼 수 있습니다.
            </blockquote>
            <v-card max-width="1000" raised class="pa-2 ma-4 mb-12">
                <v-img :src="require('../../src/assets/4-1.png')" cover class="grey darken-4 "></v-img>
            </v-card>
            ...
            <blockquote class="mb-8 blockquote body-1">
                ...
                (앱카드결제, 간편결제, 정기결제(첫결제)에 모두 적용됩니다.)
            </blockquote>
...
<!-- 
            ...
            <v-card max-width="1000" raised class="pa-2 ma-4 mb-12">
                <v-img :src="require('../../src/assets/4-3.png')" cover class="grey darken-4 "></v-img>
            </v-card>
            ...
            <blockquote class="blockquote body-1">
                자바스크립트를 이용하여 결제를 하기 위해서는 PAY와 CERT방식중에 하나를 선택하여야 하지만 빌링키 생성을 위해 빌링키 등록(AUTH)만 수행할 경우에는
                실제 결제가 일어나는 것이 아니므로 AUTH를 선택하여 빌링키만 발급받습니다.
            </blockquote>
            <v-card max-width="1000" raised class="pa-2 ma-4 mb-12">
